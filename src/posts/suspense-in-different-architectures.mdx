...
<Info>
  Note that the current generation of SSR frameworks do _not_ support selective
  hydration - as far as I can tell, only Next.js applications using the `app`
  directory support selective hydration for client-components rendered to html
  on the server. Check out my blog post on [SSR vs Server
