...
abstract:
  "Stitching together the fzf, the quickfix list, cdo, and a custom lua function for an awesome find
  and replace in Neovim"
...
fzf is an awesome command line utility for fuzzy finding just about anything (seriously, anything).
In our case, we'll be using the `fzf-lua` package for Neovim, and focusing on just one specific
command: `grep`
