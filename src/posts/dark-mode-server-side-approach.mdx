...
- Next, we grab `req` and `res` from `context.ctx` and check if they're defined. This is a crucial
  ...
  instance of `isDarkModeCookie` returned from the first call on the server. Strange, right?
- To set the cookie, we set the `Set-Cookie` header using the
  ...
  decided to go with it since it has ~48 million (!) weekly downloads, and it gets the job done
