...
<Info>
__Update__: I've since converted all my pages to use server side rendering (SSR) to allow for a more seemless dark mode experience (i.e. without a white flash).
...
into the perils of dark-mode. Thankfully, Next.js has great support for SSR as well, and I haven't yet regreted my choice in framework.
</Info>
...
For my basic continuous integration, I wrote a deploy script that builds the site locally to make sure everything's good, run a handfull of end-to-end tests 
with playwright, back up my database to my local machine, push to github, and push to the server. 
