...
But instead of manually passing in `TAccountTypeGeneric`, we can simplify our type function to _infer_ its value by specifying
...
This looks great, but if we type our component's `props` as `TProps`, how is typescript supposed to know which of the 
