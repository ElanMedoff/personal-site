...
fzf is an awesome command line utility for fuzzy finding just about anything (seriously, anything). In our case, we'll be using the `fzf-lua` package for Vim, and focusing on just one specific command: `grep`
...
<Image src="/blog/bfq-initial.png" />

## Narrow Down With bfq

The second plugin we'll use is [bfq](https://github.com/kevinhwang91/nvim-bqf), an awesome utility for the quickfix list. I won't dive into all the details, but some of the highlights are:
...
Our custom user command will accept two arguments, a `foo` and a `bar`, and execute a Vim command that interpolates the values into a string of the form `:cdo s/foo/bar/` and saves each file. Let's look at the following code:
