...
MyApp.getInitialProps = async (
  ...
  const isDarkModeCookie = cookie.parse(req.headers.cookie || "")
    .isDarkMode as string | undefined;
...
While we _could_ wrap every page in the `pages` directory with a `<ThemeContext.Provider />`, it's more ergonomic to write a custom `pages/_app.tsx` to handle this for us.
...
- Next, we grab `req` and `res` from `context.ctx` and check if they're defined. This is a crucial check: although `getInitialProps` runs on the server during the first page load, it'll also run on the _client_ during client-side navigations! Thankfully, though, since client-side state is preserved during client-side transitions, `isDarkModeCookie` is effectively ignored when `getInitialProps` runs on the client since the `isDarkMode` state is already initialized with the instance of `isDarkModeCookie` returned from the first call on the server. Strange, right? 
- To set the cookie, we set the `Set-Cookie` header using the [cookie](https://www.npmjs.com/package/cookie) npm package. Although it's a little barebones, I decided to go with it since it has ~48 million (!) weekly downloads, and it gets the job done
