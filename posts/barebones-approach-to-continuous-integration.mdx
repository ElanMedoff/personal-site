...
Of the many domains of software engineering, devops is one area I'm less familiar with. I don't have a particular interest in tinkering with linux or bash scripts, but since shipping my blog and a few hobby sites to the world, I've needed to pick up the basics in managing a deploy process. In fact, I'd say my general lack of expertise makes me a great person to write on the topic of minimalist CI setups - I know just enough to do what I need with nothing too fancy to hold me back. I've done a good deal of trial and error, and I think this is a great approach for small/medium sized projects. 

Just a final note: this setup is based on the deploy process for my blog, a Next.js app, but the fundamentals should apply to most projects.
...
<Info>
__Note__: Other approaches to CI frequently use a process that runs on a _remote_ machine, like Github actions or a Jenkins setup. These are great options for larger projects/companies, but in my opinion, a simple setup of running your build and tests locally before pushing to production works just fine. 
</Info>
...
I won't write too much on this topic since I really just use the approach written in this [article](https://toroid.org/git-website-howto), but I will add that for whatever directory you create to check out the latest tree into (i.e `/var/www/www.example.org`) _make sure you set the correct permissions_! Git needs to be able to write to that directory, so you may need to use `chmod`.
...
Like the article suggests, in my `post-receive` hook I check out the latest tree into my project folder, but I also added a line to execute a separate `post-receive.sh` script checked into the repo. See below:
...
I'm telling pm2 to reload a process called `blog` by running the command `npm run start` - and if `blog` doesn't exist yet, `pm2` will create it for us.
...
This setup works great for small projects with just a single branch (master), but for more complex apps, consider adding a step where you do something like:
...
The key is to merge into master _before_ running your build and tests, so you can have confidence in the code you'd like to push to the server.
