...
<Info>
While `getInitialProps` _should_ run on the server for the initial page load, I've found that for components using `getStaticProps`, the cookies are omitted in the first request. I have yet to figure out exactly why this happens, but in the meantime, I've been using `getServerSideProps` in all of my pages as a workaround.
</Info>
